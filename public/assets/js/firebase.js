...
try {
    ...
    onAuthStateChanged(auth, async (user) => {
        window.currentFirebaseUser = user || null;
        const adminRouteActive = window.location.pathname === '/admin' || window.location.hash === '#admin';

        ensureAnonymousSession(user);

        if (!user || user.isAnonymous) {
            clearAdminSessionMemory();
            if (adminRouteActive && typeof window.openAdminPanelModal === 'function') {
    ...
    onSnapshot(doc(db, 'settings', 'schedule_blocks'), (docSnap) => {
        if (docSnap.exists()) {
            ...
            // Jornada del estudio. Vive en este mismo documento a propósito:
            // es el que ya gobierna la agenda, y una segunda fuente de horario
            // acabaría divergiendo de la primera sin que nadie lo note.
            if (data.openingTime || data.closingTime) {
                localStorage.setItem('ncv_business_hours', JSON.stringify({
                    openingTime: data.openingTime || '08:00',
                    closingTime: data.closingTime || '20:00'
                }));
            }
            if (typeof window.loadAdminBusinessHours === 'function' && window.isAdminAuthenticated) window.loadAdminBusinessHours();
            if (typeof window.renderCalendar === 'function') window.renderCalendar();
            if (typeof window.renderTimeSlots === 'function') window.renderTimeSlots();
